...
public class TokenizedLine {
    ...
    private static func applyThemeAttributes(_ attributes: [ThemeAttribute], toStr attributedString: NSMutableAttributedString, withStyle style: EditorMutableParagraphStyle, andRange range: NSRange) {
    ...
    public func applyTheme(
        ...
        var style = EditorMutableParagraphStyle()
        if applyBaseAttributes {
            attributedString.setAttributes(nil, range: NSRange(location: loc, length: length))
        }
        else if let currStyle = (attributedString.attribute(.paragraphStyle, at: loc, effectiveRange: nil) as? EditorParagraphStyle)?.mutableCopy() as? EditorMutableParagraphStyle {
            style = currStyle
        }
